...
import { StudentFullData, SchoolSettings, AppDatabase } from '../types';
...
export function generateStudentReportPDF(
  ...
  doc.setFont('helvetica', 'bold');
  doc.text(settings.principal_name, sigCol1, sigNameY);
  doc.setFont('helvetica', 'normal');
  doc.text(`NIP. ${settings.principal_nip}`, sigCol1, sigNameY + 4);

  doc.setFont('helvetica', 'bold');
  doc.text(settings.homeroom_teacher_name, sigCol2, sigNameY);
  doc.setFont('helvetica', 'normal');
  doc.text(`NIP. ${settings.homeroom_teacher_nip}`, sigCol2, sigNameY + 4);

  // Simpan file PDF
  const safeName = s.full_name.replace(/[^a-zA-Z0-9]/g, '_');
...
export function generateClassRecapPDF(
  ...
  if (finalY < 185) {
    ...
    doc.setFont('helvetica', 'bold');
    doc.text(settings.homeroom_teacher_name, col2, finalY + 22);
    doc.setFont('helvetica', 'normal');
